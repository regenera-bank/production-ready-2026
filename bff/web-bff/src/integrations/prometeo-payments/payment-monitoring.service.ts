...
export class PaymentMonitoringService {
  ...
  listMonitored(): MonitoredPaymentRecord[] {
    ...
    return Object.values(snapshot.prometeoPayments ?? {}).sort((a, b) =>
      b.webhookReceivedAt.localeCompare(a.webhookReceivedAt),
    );
  }
  ...
  private persist(record: MonitoredPaymentRecord): void {
    this.store.mutate((draft) => {
      if (!draft.prometeoPayments) {
        draft.prometeoPayments = {};
      }
      draft.prometeoPayments[record.requestId] = record;
      if (!draft.prometeoProcessedEventIds) {
        draft.prometeoProcessedEventIds = {};
      }
  ...
  private getByRequestId(requestId: string): MonitoredPaymentRecord | undefined {
    return this.store.get().prometeoPayments?.[requestId];
  }

  private isEventProcessed(eventId: string): boolean {
    return Boolean(this.store.get().prometeoProcessedEventIds?.[eventId]);
  }
