...
export class PaymentEngineService {
  ...
  /** Liquidação idempotente — replay de idempotency-key não quebra em SETTLED. */
  async ensureSettled(paymentId: string): Promise<PaymentRecord> {
    let payment = await this.requirePayment(paymentId);
    if (payment.status === PaymentStatus.SETTLED) {
      return payment;
    }
    if (canTransitionPayment(payment.status, PaymentStatus.SENT)) {
      payment = await this.markSent(paymentId);
    }
    if (payment.status === PaymentStatus.SENT) {
      return this.markSettled(paymentId);
    }
    throw new StateTransitionException(
      'Pagamento não pode ser liquidado',
      'PAYMENT_ENSURE_SETTLE_FAILED',
