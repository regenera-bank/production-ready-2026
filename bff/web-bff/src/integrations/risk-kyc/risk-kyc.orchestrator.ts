...
import { isBaselineModeEnabled } from '../../baseline/baseline.guard';
...
export class RiskKycOrchestrator {
  ...
  async runCadastralStep(input: CadastralKycInput): Promise<CadastralKycResult> {
    ...
    let cepLookup: { state: string; city: string };
    try {
    ...
    const diditKyc = isDiditKycProvider();
    let identitySource = homologKyc() ? 'firebase-homolog' : diditKyc ? 'didit' : undefined;
    if (!homologKyc() && !diditKyc) {
    ...
    return {
      kycId: step1.kycId,
      kycStep: diditKyc ? 'didit_verification' : 'document',
      kycStatus: 'IN_REVIEW',
      identitySource,
      pepScore: step1.pepScore,
    };
