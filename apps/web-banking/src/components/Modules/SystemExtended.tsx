...
import type { ModuleId, UserProfile, AiSettings, OrbTheme, ChatMessage } from '../../types';
import { chatWithRaphaela } from '../../services/geminiService';
import {
    ...
    return (
        <div className="p-6 animate-in slide-in-from-bottom duration-500 pb-32">
            ...
            <div className="mt-8">
                ...
                <MenuItem icon={User} label="Agência" value={user.agency ?? '—'} />
                <MenuItem icon={Shield} label="Conta" value={user.accountNumber ?? '—'} />
                <MenuItem icon={Settings} label="Disponível" value={user.availableBalance.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} />
            </div>
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: `e-${Date.now()}`,
          role: 'ai',
          text: 'Falha ao contatar o BFF. Verifique GEMINI_API_KEY e a sessão.',
          timestamp: Date.now(),
        },
      ]);
    } finally {
