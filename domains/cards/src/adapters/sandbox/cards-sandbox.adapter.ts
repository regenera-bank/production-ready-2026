...
interface SandboxAuthorization {
  authId: string;
  cardId: string;
  amountCents: string;
  merchant: string;
  status: 'authorized' | 'captured' | 'reversed' | 'chargeback';
...
export class CardsSandboxAdapter implements CardsPort {
  ...
  private readonly authorizations = new Map<string, SandboxAuthorization>();
  ...
  private dispatch(action: string, command: CardsCommand): CardsResult {
    ...
    switch (action) {
      ...
      case 'issue_card': {
        const limitCents = String(command.payload.limitCents ?? '300000');
        if (!/^\d{1,19}$/.test(limitCents)) {
          return this.reject(command, 'INVALID_LIMIT');
        }
        const issued: SandboxCard = {
          id: `issued-${randomUUID()}`,
          alias: String(command.payload.alias ?? 'Regenera Crédito'),
          number: `5502 7700 ${randomUUID().replace(/-/g, '').slice(0, 4)} ${randomUUID().replace(/-/g, '').slice(0, 4)}`,
          holder: 'TITULAR CONTA',
          expiry: `03/${String((new Date().getFullYear() + 3) % 100).padStart(2, '0')}`,
          cvv: '---',
          limitCents,
          usedCents: '0',
          brand: 'mastercard',
        ...
        cards.push(issued);
        return this.accept(command, { card: issued });
      }
      case 'authorize_purchase': {
        const cardId = String(command.payload.cardId ?? '');
        const amountCents = String(command.payload.amountCents ?? '');
        const merchant = String(command.payload.merchant ?? 'merchant');
        if (!/^\d{1,19}$/.test(amountCents) || BigInt(amountCents) <= 0n) {
          return this.reject(command, 'INVALID_AMOUNT');
        }
        const card = cards.find((c) => c.id === cardId);
        if (!card || card.status !== 'active') {
          return this.reject(command, 'CARD_NOT_AVAILABLE');
        }
        const used = BigInt(card.usedCents);
        const limit = BigInt(card.limitCents);
        const amount = BigInt(amountCents);
        if (used + amount > limit) {
          return this.reject(command, 'LIMIT_EXCEEDED');
        }
        const authId = `auth-${randomUUID()}`;
        const auth: SandboxAuthorization = {
          authId,
          cardId,
          amountCents,
          merchant,
        ...
        return this.accept(command, { authorization: auth });
      }
      case 'capture_purchase': {
        const cardId = String(command.payload.cardId ?? '');
        const authId = String(command.payload.authId ?? '');
        const auth = this.authorizations.get(authId);
        if (!auth || auth.cardId !== cardId || auth.status !== 'authorized') {
          return this.reject(command, 'AUTH_NOT_FOUND');
        }
        const card = cards.find((c) => c.id === cardId);
        if (!card) {
          return this.reject(command, 'CARD_NOT_FOUND');
        }
        card.usedCents = String(BigInt(card.usedCents) + BigInt(auth.amountCents));
        const tx: SandboxCardTransaction = {
          id: randomUUID(),
          cardId,
          title: auth.merchant,
        ...
        const invoice = this.ensureInvoice(principalId, cardId);
        invoice.totalCents = String(BigInt(invoice.totalCents) + BigInt(auth.amountCents));
        auth.status = 'captured';
        auth.transactionId = tx.id;
        return this.accept(command, { transaction: tx, authorization: auth });
      }
      case 'reverse_purchase': {
        const cardId = String(command.payload.cardId ?? '');
        const authId = String(command.payload.authId ?? '');
        const auth = this.authorizations.get(authId);
        if (!auth || auth.cardId !== cardId || auth.status !== 'captured') {
          return this.reject(command, 'CAPTURE_NOT_FOUND');
        }
        const card = cards.find((c) => c.id === cardId);
        if (!card) {
          return this.reject(command, 'CARD_NOT_FOUND');
        }
        card.usedCents = String(
          BigInt(card.usedCents) > BigInt(auth.amountCents)
            ? BigInt(card.usedCents) - BigInt(auth.amountCents)
            : 0n,
        );
        const tx: SandboxCardTransaction = {
        ...
        this.transactions.set(key, list);
        const invoice = this.ensureInvoice(principalId, cardId);
        invoice.totalCents = String(
          BigInt(invoice.totalCents) > BigInt(auth.amountCents)
            ? BigInt(invoice.totalCents) - BigInt(auth.amountCents)
            : 0n,
        );
      ...
      case 'chargeback_purchase': {
        ...
        const tx: SandboxCardTransaction = {
          id: randomUUID(),
          cardId,
          title: `Chargeback — ${auth.merchant}`,
          amountCents: `-${auth.amountCents}`,
          date: new Date().toISOString(),
          category: 'chargeback',
        };
        ...
        list.unshift(tx);
        this.transactions.set(key, list);
        auth.status = 'chargeback';
        return this.accept(command, { transaction: tx, chargebackId: tx.id });
      }
      default:
