...
import { fetchSuitability } from '../../platform/bff-client';
import {
  balanceTrend,
  sumOutflows,
...
export const LoansModule: React.FC<BankingModuleProps> = ({
  user,
  accessToken,
  onNavigate,
}) => {
  ...
  useEffect(() => {
    ...
    void fetchSuitability(accessToken)
  ...
  return (
    <div className="p-6 space-y-8 animate-in slide-in-from-right duration-500 pb-32">
      <p className="text-[10px] text-amber-400/90 uppercase tracking-widest text-center border border-amber-500/20 rounded-full py-2 px-4">
        Sandbox crédito · score provisório de GET /products/investments/suitability
      </p>
        ...
        <div className="flex justify-between items-start relative z-10">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-2">
              Fonte BFF suitability · domínio crédito (SCD) pendente
            </p>
          </div>
...
export const InsuranceModule: React.FC<BankingModuleProps> = ({ user, transactions, onNavigate }) => {
  ...
  return (
    <div className="p-6 space-y-6 animate-in slide-in-from-right duration-500 pb-32">
      ...
      >
        Abrir protocolo via suporte Raphaela
      </button>
    </div>
  );
};