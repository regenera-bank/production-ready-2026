...
readonly DB_NAME="regenera_bank_prod"
readonly DB_USER="regenera_backup_user" # Usuário com permissões de LEITURA apenas.
readonly DB_HOST="regenera-prod-cluster.c1example.us-east-1.rds.amazonaws.com"

# Tier 1: Local backup for fast recovery.
readonly LOCAL_BACKUP_DIR="/mnt/backups/daily"
...
notify_failure() {
  local message="FATAL: Regenera Bank backup process failed. Needs immediate attention. Stage: $1"
  log "$message"
  # TODO: Integrate with PagerDuty.
  # curl -X POST -H 'Content-type: application/json' --data '{"routing_key":"<PAGERDUTY_KEY>","event_action":"trigger","payload":{"summary":"'$message'","source":"backup-script","severity":"critical"}}' https://events.pagerduty.com/v2/enqueue
}
...
check_dependencies() {
  ...
  if ! command -v pg_dump &> /dev/null; then
    ...
    log "FATAL: pg_dump could not be found."
    exit 1
  fi
  if ! command -v aws &> /dev/null; then
    notify_failure "Dependency Check"
    log "FATAL: aws-cli could not be found."
    exit 1
  fi
...
create_local_backup() {
  ...
  # Set the password for pg_dump from an environment variable for security.
...
upload_to_primary_s3() {
  ...
  log "Tier 2 complete. Backup uploaded to primary S3 bucket."
}
...
upload_to_dr_s3() {
  ...
  if [ $? -ne 0 ]; then
    ...
    # Don't exit here. A DR failure is critical but the primary backup succeeded.
    # We send an alert and continue. The system is still recoverable.
  else
    log "Tier 3 complete. Backup uploaded to DR S3 bucket."
  fi
}
...
# This DB_PROD_PASSWORD should be set by the cron job's execution environment.
