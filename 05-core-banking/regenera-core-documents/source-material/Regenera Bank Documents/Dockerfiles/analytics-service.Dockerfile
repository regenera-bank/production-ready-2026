# [FILE] apps/services/analytics-service/Dockerfile
# Dockerfile Canônico para Microserviços NestJS do Regenera Bank
# Adaptado para: analytics-service
...
RUN pnpm install --prod --filter="analytics-service..."
COPY . .
...
COPY --from=builder /usr/src/app/apps/services/analytics-service/dist ./dist
EXPOSE 3009
