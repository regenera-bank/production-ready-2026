...
export class PixEngineService {
  ...
  async create(command: CreatePixCommand): Promise<PixPaymentRecord> {
    ...
    // Replay idempotente (§21): payments.create devolveu um pagamento já
    // existente para esta Idempotency-Key. O comprovante deve ser ESTÁVEL —
    // devolve o registro Pix original com o MESMO endToEndId, sem gerar novo.
    const replayed = await this.pix.findByPaymentId(payment.id);
    if (replayed) {
      return replayed;
    }
  ...
  async findByPaymentId(paymentId: string): Promise<PixPaymentRecord | null> {
    return this.pix.findByPaymentId(paymentId);
  }
