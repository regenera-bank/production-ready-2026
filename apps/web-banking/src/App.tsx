...
import { ModuleId, Transaction, UserProfile, AiResponse, ChatMessage, AiSettings, OrbTheme, MapLocation } from './types';
import { chatWithRaphaela, speakWithRaphaela } from './services/geminiService';
...
import { saveSession, loadSession, clearSession } from './platform/session';
...
import RaphaelaOrb from './components/AI/RaphaelaOrb';
import Manifesto from './Manifeste/manifeste';
...
const App: React.FC = () => {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { id: '0', role: 'ai', text: 'Assistente Raphaela pronta. Como posso ajudar com sua conta?', timestamp: Date.now() }
  ]);
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      if (isFirebaseConfigured() && auth && !token.startsWith('homolog-')) {
  ...
  const executeRealCloud = async (params: any) => {
     if (!aiSettings.googleCloudToken) {
         showFeedback('Token Google Cloud não configurado — integração indisponível.', 'alert');
         return;
     }
     
     try {
         console.log(`[RAPHAELA CLOUD] Executing: ${params.service}.${params.action} on ${params.resource}`);
         
         if (params.service === 'compute' && params.action === 'list') {
             const response = await fetch(`https://compute.googleapis.com/compute/v1/projects/${userProfile.name.toLowerCase().replace(' ', '-')}/zones/us-central1-a/instances`, {
                 headers: { 'Authorization': `Bearer ${aiSettings.googleCloudToken}` }
             });
             if (response.ok) {
                 const data = await response.json();
                 showFeedback(`Cloud: ${data.items?.length || 0} instâncias encontradas.`, 'success');
             } else {
                 showFeedback(`Falha Cloud API (Status ${response.status})`, 'alert');
             }
         } else {
             showFeedback(`Comando Cloud Autorizado: ${params.action} ${params.service}`, 'success');
         }
     } catch {
         showFeedback(`Erro na execução Cloud API`, 'alert');
     }
  };

  // --- AUDIO HANDLING (GEMINI TTS) ---
  ...
  const handleSendMessage = async (text: string) => {
    ...
    const response: AiResponse = await chatWithRaphaela(text, context, aiSettings, accessToken);

    setIsAiThinking(false);
    setThinkingModel(null);
    
    setChatMessages(prev => [...prev, { id: Date.now().toString(), role: 'ai', text: response.text, timestamp: Date.now() }]);
    ...
    if (response.action && response.action !== 'none') {
        switch (response.action) {
            case 'navigate': {
                const moduleId = response.params?.module;
                if (typeof moduleId === 'string') {
                  handleNavigate(moduleId as ModuleId);
                }
                break;
            }
            case 'pix_send':
            ...
            case 'toggle_balance':
                ...
                break;
            case 'change_theme': {
                const theme = response.params?.theme;
                if (typeof theme === 'string') {
                    const orbTheme = theme as OrbTheme;
                    setAiSettings((prev) => ({ ...prev, orbTheme }));
                    showFeedback(`Espectro ${orbTheme} sincronizado.`, 'success');
                }
                break;
            }
            case 'search_transactions': {
                const query = response.params?.query;
            ...
            case 'send_telegram': {
                ...
                if (typeof message === 'string') {
                  executeRealTelegram(message);
                }
                break;
            }
            case 'manage_cloud':
                executeRealCloud(response.params);
                break;
            case 'send_email':
                if (aiSettings.googleCloudToken && response.params?.to) {
                    try {
                        const to = response.params.to;
                        const subject = response.params.subject || 'Mensagem via Regenera AI';
                        const body = response.params.body || 'Enviado por Raphaela.';
                        ...
                        showFeedback(`Enviando via Gmail API...`, 'success');
                        
                        fetch('https://gmail.googleapis.com/gmail/v1/users/me/messages/send', {
                            method: 'POST',
                            headers: {
                                'Authorization': `Bearer ${aiSettings.googleCloudToken}`,
                                'Content-Type': 'application/json'
                            },
                            body: JSON.stringify({ raw: encodedMessage })
                        }).then(res => {
                            if (res.ok) showFeedback(`E-mail REAL enviado para ${to}`, 'success');
                            else showFeedback(`Falha Gmail API (Status ${res.status}). Verifique o Token.`, 'alert');
                        }).catch(() => {
                            showFeedback('Erro de conexão com Gmail API.', 'alert');
                        });
                    } catch {
                         showFeedback('Erro ao processar envio de e-mail.', 'alert');
                    }
                } else {
                    showFeedback('Token Gmail não configurado — e-mail não enviado.', 'alert');
                }
                break;
        }
    }

    const audioData = await speakWithRaphaela(response.text, aiSettings.voice, accessToken);
    if (audioData) {
        playAudio(audioData);
    }
  };
  ...
  const handleThemeCycle = () => {
      ...
      const currentIndex = themes.indexOf(aiSettings.orbTheme);
      const nextIndex = (currentIndex + 1) % themes.length;
      setAiSettings(prev => ({ ...prev, orbTheme: themes[nextIndex] }));
  };

  const renderAiChat = () => (
    <div className={`absolute inset-0 z-[70] transition-all duration-500 ${showAiChat ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => setShowAiChat(false)}></div>
        <div className={`absolute bottom-0 left-0 right-0 h-[70vh] bg-bg-deep border-t border-white/10 rounded-t-[3rem] transform transition-transform duration-500 ${showAiChat ? 'translate-y-0' : 'translate-y-full'} flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.5)]`}>
            <div className="p-6 border-b border-white/5 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-cyan-400 font-bold uppercase tracking-widest">Gemini via BFF</p>
                    </div>
                </div>
                <button onClick={() => setShowAiChat(false)} className="p-2 bg-white/5 rounded-full text-gray-400 hover:text-white transition-colors"><X className="w-5 h-5" /></button>
            </div>
            ...
            <aside className="side-menu-panel flex flex-col" role="dialog" aria-modal="true" aria-label="Menu principal">
                ...
                <div className="flex-1 overflow-y-auto p-2 custom-scrollbar pb-20">
                    ...
                    <MenuSection title="Patrimônio">
                        ...
                        <MenuItem id="loans" icon={DollarSign} label="Crédito" />
  ...
  if (bootstrapping) {
    return (
      <AppShell className="font-sans text-white overflow-hidden">
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-bg-deep">
          <div className="w-10 h-10 border-2 border-primary border-t-transparent rounded-full animate-spin" />
          <p className="text-xs text-gray-400 uppercase tracking-widest">Carregando sessão...</p>
        </div>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell withCosmic className="font-sans text-white overflow-hidden flex flex-col">
      ...
      {currentSearchResults.length > 0 && !isAiThinking && (
      ...
      {activeModule === 'transactions' && (
          <div className="screen-transition module-view">
               ...
               <TransactionsModule globalSearchQuery={globalSearchTerm} transactions={transactions} />
          </div>
      )}
      ...
      <footer className="bottom-nav-surface absolute bottom-0 left-0 right-0 backdrop-blur-md border-t flex items-center justify-between z-50 safe-bottom">
        <div className="flex justify-around items-center w-full max-w-4xl mx-auto">
            ...
            <div className="orbit-dock relative">
                <RaphaelaOrb isListening={isListening} isThinking={isAiThinking} isSpeaking={isAiSpeaking} onClick={() => setShowAiChat(true)} theme={aiSettings.orbTheme} onThemeCycle={handleThemeCycle} />
