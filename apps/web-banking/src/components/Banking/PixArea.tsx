...
import { formatAmountInputBrl } from '../../platform/money';
...
const PixArea: React.FC<PixAreaProps> = ({
  ...
  onOperationComplete,
    ...
    // --- SEND PIX STATE ---
    const [sendStep, setSendStep] = useState<'form' | 'processing' | 'pending' | 'success'>('form');
    const [showConfirmModal, setShowConfirmModal] = useState(false); // New explicit modal state
    // Chave de idempotência estável por intenção de pagamento: retry visual
    // reutiliza a MESMA chave (gate §48 — operação duplicada é reprovação).
    const idempotencyKeyRef = React.useRef<string | null>(null);
    const [lastPaymentId, setLastPaymentId] = useState<string | null>(null);
    const [lastEndToEndId, setLastEndToEndId] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        if (cleanKey.length < 5 || !accessToken) {
            setContactName('Destinatário...');
            setContactInstitution('—');
            return;
        }
        const timer = setTimeout(() => {
            lookupPixKey(accessToken, cleanKey)
              .then((result) => {
                setContactName(result.displayName ?? 'Destinatário externo');
                setContactInstitution(result.institution ?? 'Instituição parceira');
              })
              .catch(() =>
                lookupPixExternal(accessToken, cleanKey)
                  .then((ext) => {
                    if (ext.found) {
                      setContactName(ext.displayName ?? 'Destinatário SPI');
                      setContactInstitution(ext.institution ?? 'Participante externo');
                    } else {
                      setContactName('Destinatário...');
                      setContactInstitution('—');
                    }
    ...
    useEffect(() => {
        ...
        fetchPixKeys(accessToken)
          .then((res) => setMyKeys(res.items))
          .catch(() => setMyKeys([]));
    }, [accessToken]);

    const pixHistory = transactions.filter((t) =>
      t.title.toLowerCase().includes('pix'),
    );

    const recentPixContacts: Contact[] = pixHistory
      .filter((t) => t.type === 'outflow' && t.party)
      .slice(0, 5)
      .map((t, index) => ({
        id: `tx-${t.id}-${index}`,
        name: t.party,
    ...
    const primaryPixKey = myKeys[0]?.key ?? '';

    const receiveShareText = primaryPixKey
      ? receiveAmount
        ? `Chave Pix Regenera: ${primaryPixKey} — valor sugerido R$ ${receiveAmount}`
        : `Chave Pix Regenera: ${primaryPixKey}`
      : '';
    ...
    /**
     * Faz poll do paymentId até estado terminal. Nunca declara sucesso em
     * PROCESSING; após esgotar tentativas, mostra estado "pending" honesto.
     */
    const pollUntilSettled = async (paymentId: string, firstDelayMs: number) => {
        let delay = Math.max(firstDelayMs, 400);
        for (let attempt = 0; attempt < 15; attempt += 1) {
            await new Promise((resolve) => setTimeout(resolve, delay));
            try {
                const status = await getPixTransferStatus(accessToken, paymentId);
                if (status.status === 'SETTLED' || status.status === 'RECONCILED') {
                ...
                if (status.status === 'FAILED') {
                    idempotencyKeyRef.current = null;
                    setPixError('O Pix foi rejeitado. Nenhum valor foi debitado.');
                    setSendStep('form');
                    return;
                }
                delay = Math.max(status.pollAfterMs ?? delay, 400);
            } catch {
                // Rede instável durante poll: manter tentativa — o pagamento
                // já foi aceito pelo core; o estado é consultável a qualquer momento.
                delay = Math.min(delay * 2, 5000);
            }
        }
        // Não liquidou dentro da janela de acompanhamento: estado honesto.
        await onOperationComplete();
        setSendStep('pending');
    };
    ...
    const handleConfirmSend = async () => {
        ...
        // Reutiliza a chave da intenção atual em retry; só gera nova quando
        // uma intenção anterior foi concluída ou descartada.
        if (!idempotencyKeyRef.current) {
            idempotencyKeyRef.current =
              (globalThis.crypto?.randomUUID?.() ?? `pix-ui-${Date.now()}`);
        }
        try {
            ...
            setLastEndToEndId(result.endToEndId);
            if (result.status === 'PROCESSING' || result.status === 'UNKNOWN') {
                await pollUntilSettled(result.paymentId, result.pollAfterMs);
                return;
            }
            ...
            // Timeout/erro de rede APÓS envio: não regenerar chave — o retry
            // com a mesma Idempotency-Key devolve o mesmo resultado.
    ...
    const handleDeleteKey = () => {
        setPixError('Exclusão de chave Pix ainda não exposta na API do BFF');
    };

    const handleAddKey = async () => {
        if (!newKeyInput || !accessToken) return;
        try {
            const created = await registerPixKey(accessToken, newKeyInput, newKeyType);
            setMyKeys((prev) => [...prev, created]);
            setIsAddingKey(false);
            setNewKeyInput('');
        } catch (error) {
            setPixError(error instanceof Error ? error.message : 'Falha ao cadastrar chave');
        }
    };
    ...
    return (
        <div className="p-6 animate-in slide-in-from-right duration-500 min-h-[70vh] relative module-view">
            {renderTabs()}
                    ...
                    <div className="bg-gradient-to-br from-bg-mid to-bg-deep border border-cyan-500/30 w-full max-w-md rounded-[2rem] p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)] relative overflow-hidden animate-in zoom-in-95 duration-300">
                        ...
                        <div className="bg-white/5 rounded-2xl p-6 space-y-4 mb-8 border border-white/5">
                            <div className="flex justify-between items-end border-b border-white/5 pb-4">
                                ...
                                <span className="text-3xl font-bold text-white tracking-tight">R$ {formatAmountInputBrl(amount)}</span>
                            </div>
                            <div className="space-y-2 pt-2">
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="text-white">{contactInstitution}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                ...
                <div className="animate-in fade-in">
                    {sendStep === 'form' && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 shadow-xl relative overflow-hidden group hover:border-cyan-500/20 transition-all">
                                ...
                                {pixError && (
                                  <p className="text-xs text-red-400 mb-4 text-center">{pixError}</p>
                                )}
                                ...
                                <div className="mt-6 relative z-10">
                                    ...
                                    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                                        {recentPixContacts.length === 0 && (
                                          <p className="text-[10px] text-gray-600">Nenhum Pix recente no extrato</p>
                                        )}
                                        {recentPixContacts.map(contact => (
                                            <span
                                                key={contact.id}
                                                className="flex items-center gap-2 bg-white/5 border border-white/5 rounded-full px-3 py-1.5 whitespace-nowrap"
                                            >
                                                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center text-[10px] font-bold text-white">
                    ...
                    {sendStep === 'processing' && (
                        <div className="flex flex-col items-center justify-center h-[50vh] text-center animate-in fade-in">
                            ...
                            {lastPaymentId && (
                                <p className="text-[10px] text-gray-600 mt-4 font-mono">paymentId: {lastPaymentId}</p>
                            )}
                        </div>
                    )}
                    ...
                    {sendStep === 'pending' && (
                        <div className="flex flex-col items-center justify-center h-[50vh] text-center p-6 animate-in fade-in">
                            ...
                            <h2 className="text-2xl font-bold text-white mb-2">Pix em processamento</h2>
                            <p className="text-gray-400 mb-2">
                                Seu Pix de R$ {formatAmountInputBrl(amount)} foi aceito e está sendo liquidado.
                            </p>
                            <p className="text-sm text-gray-500 mb-8">
                                Você pode fechar esta tela — o resultado aparecerá no extrato.
                                {lastPaymentId && (
                                    <span className="block font-mono text-[10px] mt-2 text-gray-600">paymentId: {lastPaymentId}</span>
                                )}
                            </p>
                            <div className="flex gap-4 w-full">
                                <button
                                    onClick={() => lastPaymentId && pollUntilSettled(lastPaymentId, 400)}
                                ...
                                <button
                                    onClick={() => { setSendStep('form'); setPixKey(''); setAmount(''); idempotencyKeyRef.current = null; }}
                                    className="flex-1 py-3 bg-cyan-600 rounded-xl font-bold hover:bg-cyan-500 transition-colors"
                                >
                                    Ir para o início
                                </button>
                            </div>
                    ...
                    {sendStep === 'success' && (
                        <div className="flex flex-col items-center justify-center h-[50vh] text-center p-6 animate-in zoom-in duration-500">
                            ...
                            <p className="text-gray-400 mb-2">R$ {formatAmountInputBrl(amount)} para {contactName}</p>
                            {lastEndToEndId && (
                                <p className="text-[10px] text-gray-600 mb-6 font-mono break-all">E2E: {lastEndToEndId}</p>
                            )}
                            
                            <div className="flex gap-4 w-full">
                                <button
                                    onClick={() => handleShare('Comprovante Pix', `Pix de R$ ${amount} enviado para ${contactName} via Regenera Bank.${lastEndToEndId ? ` E2E: ${lastEndToEndId}` : ''}${lastPaymentId ? ` paymentId: ${lastPaymentId}` : ''}`)}
                                    className="flex-1 py-3 bg-white/10 rounded-xl font-bold hover:bg-white/20 transition-colors flex items-center justify-center gap-2"
                                >
                                    <Share2 className="w-4 h-4" /> Comprovante
                                </button>
                                <button onClick={() => { setSendStep('form'); setPixKey(''); setAmount(''); idempotencyKeyRef.current = null; setLastPaymentId(null); setLastEndToEndId(null); }} className="flex-1 py-3 bg-cyan-600 rounded-xl font-bold hover:bg-cyan-500 transition-colors">
                                    Novo Pix
                                </button>
                            </div>
                        </div>
                ...
                <div className="flex flex-col items-center space-y-8 animate-in fade-in pt-4">
                    {!primaryPixKey && (
                      <p className="text-sm text-amber-400/90 text-center px-4">
                        Abra a conta e aguarde o cadastro automático da chave Pix no BFF.
                      </p>
                    )}
                    <div
                      className="relative w-full max-w-sm group cursor-pointer"
                      onClick={() => primaryPixKey && handleCopy(primaryPixKey)}
                    >
                        ...
                        <div className="relative bg-bg-mid border border-white/10 p-6 rounded-[1.8rem] shadow-[0_0_50px_rgba(34,211,238,0.08)] text-center">
                            <Key className="w-10 h-10 text-cyan-400 mx-auto mb-4" />
                            <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-2">Sua chave Pix</p>
                            <p className="text-sm text-white font-mono break-all leading-relaxed">
                              {primaryPixKey || '—'}
                            </p>
                            <p className="text-[10px] text-gray-500 mt-4">
                              QR Code EMV (BACEN) ainda não integrado — use a chave cadastrada no BFF.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 w-full max-w-sm">
                        <button 
                            type="button"
                            disabled={!primaryPixKey}
                            onClick={() => primaryPixKey && handleCopy(primaryPixKey)}
                            className="w-full py-4 bg-cyan-600 hover:bg-cyan-500 rounded-xl font-bold text-white uppercase tracking-widest shadow-lg flex items-center justify-center gap-2 transition-transform active:scale-95 disabled:opacity-40"
                        >
                            <Copy className="w-4 h-4" /> Copiar chave Pix ({primaryPixKey ? 'cadastrada' : '—'})
                        </button>
                        <button
                             type="button"
                             disabled={!primaryPixKey}
                             onClick={() => receiveShareText && handleShare('Chave Pix', receiveShareText)}
                             className="w-full py-4 border border-white/10 hover:bg-white/5 rounded-xl font-bold text-gray-400 hover:text-white uppercase tracking-widest flex items-center justify-center gap-2 transition-colors disabled:opacity-40"
                        >
                            <Share2 className="w-4 h-4" /> Compartilhar chave
                        </button>
                    </div>
                </div>
            )}
